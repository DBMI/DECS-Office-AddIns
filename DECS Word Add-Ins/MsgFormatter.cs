...
namespace DecsWordAddIns
{
    ...
    {
        private readonly string[] BREAK_AFTER_PATTERNS = { @"RE:\s*\S+\s*", @"sent at \d{1,2}\/\d{1,2}\/\d{4} \d{1,2}:\d{2} [AP]M (?:\w{3} )?-----\s*", @"(Subject:\s*[\d\w\s\>\<]{1,40}\.\s+)" };
        private readonly string[] BREAK_INSIDE_PATTERNS_I = { @"(Subject:\s*[\d\w\s\>\<]{1,40})(\s(?:Dear|Good|Hello|Hi|HI|My)[\s,]+)" };
        private readonly string[] BREAK_BEFORE_PATTERNS = { @"""?----- Message", "From:", "Sent:", "To:", @"Subject:\s*(?!RE:)", "RE:", @"\d{11},", @"sent at \d{1,2}\/\d{1,2}\/\d{4} \d{1,2}:\d{2} [AP]M (?:\w{3} )?-----\s*" };
        ...
        public void Format(Document doc)
        {
            ...
            foreach (string breakInsidePattern in BREAK_INSIDE_PATTERNS_I)
            {
